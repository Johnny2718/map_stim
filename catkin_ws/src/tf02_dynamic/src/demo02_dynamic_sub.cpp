...
int main(int argc,char *argv[])
{
    ...
    ps.header.frame_id="turtle1";                                 //reference coordinate points
    ps.header.stamp=ros::Time(0.0);                                //current time
    ...
    while(ros::ok())
    {
        ...
        try 
        {
            ps_out=buffer.transform(ps,"world");                 //convert ps to coordiante that relative to world,include <tf2_geometry_msgs/tf2_geometry_msgs.h> 
